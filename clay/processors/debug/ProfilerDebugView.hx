...
class ProfilerDebugView extends DebugView {
	...
	public function start(_id:String, ?_max:Float) {
		...
		if(_item == null) {
			//create it
			_item = new ProfilerValue(_id, new ProfilerBar(world, _id, _max, new Color().from_int(0xffa563) ));
...
private class ProfilerGraph {
	...
	public var graphbg_geometry : Sprite;
	...
	public function new(world:World, _name:String, _bg:Bool=true){
	...
	public function create() {
		...
		if(bg) {
			graphbg_geometry = new Sprite({
				color : new Color().from_int(0x101010),
				world: world,
				depth : 999.3,
				layer: Clay.debug.layer,
				size: new Vector(width-segment, height2),
				pos: new Vector()
			});
		}
	...
	public function destroy() {
		...
		graphbg_geometry.destroy();
		graph_geometry.destroy();
...
private class ProfilerBar {
	...
	public function new(world:World, _name:String, ?_max:Float, _color:Color) {

		name = _name;

		graph = new ProfilerGraph(world, _name);
		graph.create();
		...
		bg_geometry = new Sprite({
			...
			depth : 999.3,
			layer: Clay.debug.layer,
			size: new Vector(graph.width, graph.height),
			pos: new Vector()
		});
		...
		bar_geometry = new Sprite({
			...
			depth : 999.33,
			layer: Clay.debug.layer,
			size: new Vector(graph.width-2, graph.height-2),
			pos: new Vector(1,1)
		});
	...
	function set_pos(_p:Vector) {
		...
		text_item.pos.set(_p.x+(graph.width*2)+10, _p.y-6);
