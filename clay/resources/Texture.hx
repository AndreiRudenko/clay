...
class Texture extends clay.resources.Resource {
	...
	public static function create(width:Int, height:Int, ?format:TextureFormat, ?usage:Usage) {
		var img = kha.Image.create(width, height, format, usage);
		return new Texture(img);
	}

	public static function createFromBytes(bytes:Bytes, width:Int, height:Int, ?format:TextureFormat, ?usage:Usage) {
		var img = kha.Image.fromBytes(bytes, width, height, format, usage);
		return new Texture(img);
	}

	public static function createRenderTarget(width:Int, height:Int, ?format:TextureFormat, ?depthStencil:DepthStencilFormat, ?antialiasing:Int, ?contextID:Int) {
