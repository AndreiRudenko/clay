...
Config.create_khafile = function(config) {
	...
	if(config.android != null) {
		...
		if(config.android.package) {
			kfile += "p.targetOptions.android_native.package = \"" + config.android.package + "\";\n";
		}
	}
	if(config.ios != null) {
		if(project.version != null) {
...
commands_Build.prototype = $extend(Command.prototype,{
	...
	,build_project: function(config) {
		...
		switch(_g2) {
		case "android-native":case "android-native-hl":
			var arch = "arm7";
			if(config.android != null) {
		...
		var res = args == null ? js_node_ChildProcess.spawnSync("node",{ shell : true, stdio : "inherit"}).status : js_node_ChildProcess.spawnSync("node",args,{ stdio : "inherit"}).status;
		if(res != 0) {
	...
	execute: function(args) {
		...
		while(_g < to_remove.length) {
			...
			if(sys_FileSystem.exists(s)) {
				if(sys_FileSystem.isDirectory(s)) {
					this.delete_dir(s);
					if(sys_FileSystem.exists(s)) {
						var _g1 = 0;
						var _g11 = js_node_Fs.readdirSync(s);
						while(_g1 < _g11.length) {
							var file = _g11[_g1];
							++_g1;
							var curPath = s + "/" + file;
...
commands_Init.prototype = $extend(Command.prototype,{
	...
	,copy_dir: function(src_dir,dst_dir) {
		...
		while(_g < _g1.length) {
			...
			if(sys_FileSystem.isDirectory(src_path)) {
...
commands_Launch.prototype = $extend(Command.prototype,{
	...
	,launch_project: function(target) {
		...
		case "android-native-hl":
			var path = haxe_io_Path.join([CLI.user_dir,"build/android-native-hl-build/" + config.project.title + "/app/build/outputs/apk/debug/app-debug.apk"]);
			if(!sys_FileSystem.exists(path)) {
				CLI.error("Can`t find app at: " + path);
			}
			var pkg = config.android.package;
			js_node_ChildProcess.execSync("start cmd /c adb install -r " + path);
			js_node_ChildProcess.execSync("start cmd /c adb shell am start -n " + pkg + "/tech.kode.kore.KoreActivity");
			js_node_ChildProcess.execSync("start cmd /c adb logcat -s kore DEBUG AndroidRuntime");
			break;
		case "html5":
			var port = 8080;
			...
			break;
		case "linux":
			var path1 = haxe_io_Path.join([CLI.user_dir,"build/linux/" + config.project.title]);
			if(!sys_FileSystem.exists(path1)) {
				CLI.error("Can`t find app at: " + path1);
			}
			js_node_ChildProcess.execSync("xdg-open " + path1);
			break;
		case "osx":
			var path2 = haxe_io_Path.join([CLI.user_dir,"build/osx/" + config.project.title]);
			if(!sys_FileSystem.exists(path2)) {
				CLI.error("Can`t find app at: " + path2);
			}
			js_node_ChildProcess.execSync("open " + path2);
			break;
		case "windows":
			var path3 = haxe_io_Path.join([CLI.user_dir,"build/windows/" + config.project.title + ".exe"]);
			if(!sys_FileSystem.exists(path3)) {
				CLI.error("Can`t find app at: " + path3);
			}
			js_node_ChildProcess.execSync("start cmd /c " + path3);
			break;
		case "windows-hl":
			var path4 = haxe_io_Path.join([CLI.user_dir,"build/windows-hl/" + config.project.title + ".exe"]);
			if(!sys_FileSystem.exists(path4)) {
				CLI.error("Can`t find app at: " + path4);
			}
			js_node_ChildProcess.execSync("start cmd /c " + path4);
			break;
		}
	}
});
...
CLI.kha_path = "Kha";
CLI.targets = ["html5","windows","windows-hl","osx","linux","android-native","android-native-hl","ios","uwp"];
